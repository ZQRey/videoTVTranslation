...
5. **Подтверждение изменения статуса (Client -> Server)**:
   ...
   ```
6. **Команда спящего режима (Standby - Server -> Client)**:
   ```json
   {
     "type": "set_standby",
     "standby": true
...
## 7. Система контроля расписания вещания (24/7 и интервалы)

Сервер включает службу `ScheduleEnforcer`, выполняющую непрерывный мониторинг времени (каждые 3 сек):
1. **Глобальный режим**:
   - `24/7`: непрерывное круглосуточное вещание.
   - `Интервал`: время включения эфира (например `07:55`) и ухода в режим ожидания (например `19:55`), а также список активных дней недели (Пн-Вс). Поддерживается переход через полночь (ночные смены).
2. **Индивидуальное переопределение для экранов**:
   - Каждый клиент (Windows, Linux, Android) может иметь собственный режим: `Серверное` (наследует общее), `24/7 Всегда` или `Свой интервал` со своими часами и днями недели.
   - Вне расписания экраны автоматически переходят в `Standby` (сплошной чёрный экран, звук заглушен, декодирование приостановлено). При наступлении времени вещания экраны мгновенно возвращаются в прямой эфир.
   - Ручной `Standby`, выставленный администратором вручную, не сбрасывается расписанием до смены фазы.
3. **Публичный телеметрический эндпоинт**:
   - `GET /api/client/status` доступен без авторизации для Android TV и ТВ-приставок, опрашивается клиентом каждые 4 секунды для синхронизации отображения.
